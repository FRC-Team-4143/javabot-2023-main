        ...
        public List<PathPlannerTrajectory> getPath() {
            ...
            if (path == null) {
                if(constraints.maxVelocity == 0) 
                 path = PathPlanner.loadPathGroup(pathName, new PathConstraints(3.0, 3.0), PathPlanner.getConstraintsFromPath(pathName));
                 else 
                path = PathPlanner.loadPathGroup(pathName, constraints);
            }
